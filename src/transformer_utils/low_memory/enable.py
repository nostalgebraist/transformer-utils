...
def low_memory_from_pretrained(pretrained_model_name_or_path, map_location="cuda:0", *args, **kwargs):
    config_path, model_path = huggingface_model_local_paths(pretrained_model_name_or_path)

    model = low_memory_load(config_path=config_path, model_path=model_path, verbose=False, high_memory_device=map_location)
